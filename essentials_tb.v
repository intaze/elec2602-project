...
module essentials_tb;
    ...
    initial begin
        ...
        $display("\n--- LDI tests ---");
        apply_instr(5'b00000, 2'b00, 16'h000A, "LDI_R0 ");

        // TEST 2 – LDI R1, 0x0005   (R1 = 5)
        apply_instr(5'b00000, 2'b01, 16'h0005, "LDI_R1 ");

        // TEST 3 – LDI R2, 0x0003   (R2 = 3)
        apply_instr(5'b00000, 2'b10, 16'h0003, "LDI_R2 ");

        // TEST 4 – MOV R2, R0 (R2 = R0 = 10) - data_in[1:0] encodes the source register index (R0 = 0)
        $display("\n--- MOV test ---");
        apply_instr(5'b00001, 2'b10, 16'h0000, "MOV_R2 ");

        // TEST 5 – ADD R0, R1       (R0 = R0 + R1 = 10 + 5 = 15)
        $display("\n--- ADD test ---");
        apply_instr(5'b00010, 2'b00, 16'h0001, "ADD_R0 ");

        // TEST 6 – SUB R0, R2       (R0 = R0 - R2 = 15 - 10 = 5)
        $display("\n--- SUB test ---");
        apply_instr(5'b00011, 2'b00, 16'h0002, "SUB_R0 ");

        // TEST 7 – ADD R1, R2       (R1 = R1 + R2 = 5 + 10 = 15)
        apply_instr(5'b00010, 2'b01, 16'h0002, "ADD_R1 ");

        // TEST 8 – SUB R2, R1       (R2 = R2 - R1 = 10 - 15 = -5)
        apply_instr(5'b00011, 2'b10, 16'h0001, "SUB_R2 ");

        // TEST 9 – LDI R0, 0x0000   (R0 = 0, boundary / zero test)
        $display("\n--- Edge-case tests ---");
        apply_instr(5'b00000, 2'b00, 16'h0000, "LDI_R0Z");

        // TEST 10 – ADD R0, R1      (0 + 15 = 15, add-to-zero)
        apply_instr(5'b00010, 2'b00, 16'h0001, "ADD_0+R1");

        // TEST 11 – SUB R0, R0      (R0 - R0 = 0, self-subtract)
        apply_instr(5'b00011, 2'b00, 16'h0000, "SUB_R0R0");
